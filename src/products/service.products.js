...
export const getProducts = async (limit, page, query, sort) => {
  try {
    ...
    if (sort !== null && sort !== "null")
      sortingOptions = { price: sort.toLowerCase() };
    ...
    if (response.payload === undefined) {
      return { message: "La base de datos no contiene productos", payload: {} };
    }
...
export const createProduct = async (productInfo) => {
  try {
    ...
    if (Object.keys(prodByCode).length !== 0)
      return {
        statusCode: 200,
        status: "error",
        message: "El producto ya se encuentra ingresado en la base de datos",
        payload: {},
      };
    ...
    return {
      statusCode: 201,
      status: "success",
      message: "El producto fue ingresado correctamente a la base de datos",
      payload: newProduct,
    };
  } catch (error) {
    throw error;
...
export const updateProduct = async (pid, updates) => {
  try {
    ...
    Object.keys(updates).forEach((key) => {
      if (updates[key] && updates[key] !== product[key])
    ...
    return {
      ...
      payload: { originalProduct: updatedProduct, update: changes },
    };
  } catch (error) {
    throw error;
  }
